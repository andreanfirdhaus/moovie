...
import { PlayerFrame, ServerSelector, EmbedControls, TmdbEpisodePanel } from './components';
...
export default function WatchPage() {
    ...
    const { title, type, id: tmdbId, tmdbId: tmdbNumericId, tmdbSeasons } = pageState;
    const isTv = type === 'tv';
    const hasTmdbSeasons = isTv && !!tmdbSeasons?.length;
    ...
    const servers: ServerOption[] = EMBED_SERVERS.map((s) => ({
        id: s.id,
        label: s.label,
    ...
    }, [activeServerId, activeSeason, activeEpisode, isTv, tmdbId]);

    const showTmdbEpisodePanel = hasTmdbSeasons;
    const showEmbedTvControls = isTv && !hasTmdbSeasons;
    const hasSidePanel = showTmdbEpisodePanel || showEmbedTvControls;
    ...
    const playerArea = (
        <div className={`w-full min-w-0 ${isTv ? 'lg:flex-1' : ''}`}>
            ...
            {playerUrl && <PlayerFrame playerUrl={playerUrl} />}
        </div>
    );
